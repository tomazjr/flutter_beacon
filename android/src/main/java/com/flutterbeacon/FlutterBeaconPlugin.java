...
public class FlutterBeaconPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler,
    ...
    private static final BeaconParser uidBeaconLayout = new BeaconParser()
            .setBeaconLayout(BeaconParser.EDDYSTONE_UID_LAYOUT);

    private static final BeaconParser tlmBeaconLayout = new BeaconParser()
    ...
    private void setupChannels(BinaryMessenger messenger, Activity activity) {
        ...
        if (!beaconManager.getBeaconParsers().contains(uidBeaconLayout)) {
            //beaconManager.getBeaconParsers().clear();
            beaconManager.getBeaconParsers().add(uidBeaconLayout);
        }
        // Detect the telemetry (TLM) frame:
        if (!beaconManager.getBeaconParsers().contains(tlmBeaconLayout)) {
            //beaconManager.getBeaconParsers().clear();
            beaconManager.getBeaconParsers().add(tlmBeaconLayout);
        }
